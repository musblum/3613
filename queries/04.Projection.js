...
let db = connect('mongodb://localhost:27017/stocks');
let selection = {growth: {$in: ['A-', 'A', "A+",'B-', "B+", 'B']},
    dividendYield: {$gt: .07}, marketCapInBillions: {$gte: .4 } };
let projection = {_id: false, symbol: true, dividendYield: true, marketCapInBillions: true, growth: 3}
let sort = {dividendYield: -1}
let json = db.reits.find(selection, projection).sort(sort).toArray();
printjson(json);
printjson({length: json.length});
